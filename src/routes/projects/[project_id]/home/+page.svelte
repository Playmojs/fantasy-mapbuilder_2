<script lang="ts">
    ...
    async function get_data(){
        ...
        main_map = await dtb.get_map(store.project_id, store.project_cache[store.project_id].head_map_id)
    }
    $effect(()=> {store.project_id; get_data()})

    async function update_project(){
        if(!project || project_title === '' || !main_map){return;}
    ...
    function confirm_delete(){
        if(!project){return;}
        store.confirm_modal={text: "(Not yet implemented, don't worry) Are you sure you want to delete?", 
        confirm_function() {console.log('mock-delete')}
        }
    }
    ...
    $effect(() => {if(project){project_title = project.name}})
